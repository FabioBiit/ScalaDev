...
object UnionQuestStileGuida extends App {

  val spark = SparkSession.builder()
    .master("local[*]")
    .appName("UnionQuestStileGuida")
    .getOrCreate()
  ...
  var cartelle = new File("src/main/resources/QuestStileGuida")
  ...
  for (c <- cartelle.listFiles.filter(_.isDirectory)) {
    ...
    val data = nome_file_split.apply(8)
    ...
    try {
      DF_appoggio.write
        ...
        .save("src/main/resources/NewQuest/QuestUnionStileGuida" + (numero + 2).toString() + ".xlsx")
    } catch {
